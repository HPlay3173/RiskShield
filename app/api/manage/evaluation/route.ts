...
export async function GET(request: Request) {
  ...
  const [cases, runs, active, policies, ruleFeedbackCases] = await Promise.all([
    ...
    db.prepare(`SELECT id, intake_id, evaluation_case_id, text, skill_ids_json, status, created_at, updated_at
      FROM riskshield_rule_feedback_cases WHERE status = 'pending_review' ORDER BY updated_at ASC LIMIT 100`).all(),
  ]);
  return controlJson({ cases: cases.results ?? [], runs: (runs.results ?? []).map((run) => ({ ...run, metrics: JSON.parse(String(run.metrics_json)), calibration: run.calibration_json ? JSON.parse(String(run.calibration_json)) : null })), activeCalibration: active, policies: policies.results ?? [], ruleFeedbackCases: ruleFeedbackCases.results ?? [] });
}
