...
export async function GET(request: Request) {
  ...
  const [sources, runs] = await Promise.all([
    db.prepare(`SELECT id, provider, label, query, endpoint, enabled, interval_minutes, last_run_at, last_status, last_message FROM riskshield_collector_sources ORDER BY enabled DESC, updated_at DESC`).all(),
    db.prepare(`SELECT id, source_id, status, fetched_count, new_count, candidate_count, message, started_at, finished_at FROM riskshield_collector_runs ORDER BY started_at DESC LIMIT 30`).all(),
  ]);
