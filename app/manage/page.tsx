  ...
  return (
    <AdminShell currentHref="/manage" principal={presentation} title="RiskShield 관리 홈" description="새 위험 표현을 모으고, 검토하고, 분석 지식으로 반영하는 전체 흐름을 한곳에서 관리합니다.">
      ...
      <section className="metricGrid" aria-label="현재 시스템 상태">
        ...
        <div className="metricCard"><span>최근 품질 평가</span><strong className="metricValue">{latestEvaluation ? `${latestEvaluation.passed}/${latestEvaluation.testCount}` : "—"}</strong><small>{latestEvaluation?.status === "passed" ? "라벨 평가 통과" : "아직 실행한 평가 없음"}</small></div>
      </section>
      <section className="managementCard workflowCard" aria-labelledby="workflow-title">
        ...
        <ol className="workflowMap">
          ...
          <li data-state={latestEvaluation ? "complete" : "waiting"}><span>5</span><div><strong>품질 평가</strong><small>라벨 데이터로 정확도 측정·점수 보정</small><Link href="/manage/evaluation">평가 시작</Link></div></li>
        </ol>
      </section>
