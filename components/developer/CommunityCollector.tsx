  ...
  return (
    ...
    <form className="communityCollector managementCard" onSubmit={saveCollector}>
      ...
      <div className="collectorFields">
        ...
        <label className="formField">{provider === "youtube" ? "공개 동영상 ID" : "검색어"}<input value={query} onChange={(event) => setQuery(event.target.value)} maxLength={240} placeholder={provider === "youtube" ? "동영상 ID를 쉼표로 최대 5개" : undefined} required /><small>{provider === "youtube" ? "여러 분야의 한국어 공개 영상 ID를 넣어 중립 댓글 표본부터 관찰합니다." : "메타 단어 검색 결과는 직접 위험 사용이 확인되기 전까지 후보가 되지 않습니다."}</small></label>
