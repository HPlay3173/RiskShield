...
- 관리자 선택 CSV를 최대 50건의 검토 후보로 올리는 로컬 브라우저 처리
...
v0.1은 외부 AI API, API 키, 로그인, 공동 편집, 유료 API 또는 별도 운영 데이터베이스를 요구하지 않습니다. 저장 API가 연결되지 않아도 현재 브라우저 세션에서 만들기·검토·가져오기·내보내기·Analyzer 검증이 작동합니다. 법률 판단이나 출처 사실을 앱이 임의로 생성하지 않으며, 외부 배포는 별도 승인 범위입니다.
